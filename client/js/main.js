		...
		if (rd && rd.chat) {
			if (images.length > 0) {
				...
				if (rd.privateChatTargetId) {
					if (rd.chat.sendPrivateMessage(rd.privateChatTargetId, 'image_private', messageContent)) {
				...
				if (rd.privateChatTargetId) {
					if (rd.chat.sendPrivateMessage(rd.privateChatTargetId, 'text_private', text)) {
	...
	setupFileSend({
		...
		onSend: (message) => {
			...
			if (rd && rd.chat) {
				...
				if (rd.privateChatTargetId) {
					if (rd.chat.sendPrivateMessage(rd.privateChatTargetId, msgWithUser.type + '_private', msgWithUser)) {
						// 添加到自己的聊天记录
