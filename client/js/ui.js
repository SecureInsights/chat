...
function encodeInvite(roomName, password) {
...
function decodeInvite(value) {
	try {
		const data = JSON.parse(base64UrlDecodeText(value));
		if (!data || data.v !== 2 || typeof data.r !== 'string') return null;
		return {
			roomName: data.r,
			password: typeof data.p === 'string' ? data.p : ''
		};
	} catch (error) {
		console.warn('Failed to decode invite:', error);
		return null;
	}
}

function legacySimpleDecrypt(encrypted) {
	if (!encrypted) return '';
	try {
		const shifted = encrypted.split('').map(char => String.fromCharCode(char.charCodeAt(0) - 3)).join('');
...
function handleShareAction() {
	...
	const invite = encodeInvite(roomName, password);
	const url = `${location.origin}${location.pathname}#invite=${encodeURIComponent(invite)}`;
...
export function createUserItem(user, isMe) {
	...
	const safeFingerprint = user.fingerprint ? escapeHTML(user.fingerprint) : '';
	div.title = safeFingerprint ? `Safety code: ${safeFingerprint}` : '';
	div.innerHTML = `<span class="avatar"></span><div class="member-info"><div class="member-name">${safeUserName}${isMe?t('ui.me', ' (me)'):''}</div>${safeFingerprint?`<div class="member-fingerprint">${safeFingerprint}</div>`:''}</div>`;
...
export function autofillRoomPwd(formPrefix = '') {
	...
	// Backward compatibility for old query formats.
	...
	if (invite) {
		const decoded = decodeInvite(decodeURIComponent(invite));
		if (decoded) {
			roomValue = decoded.roomName;
			pwdValue = decoded.password;
		}
	} else if (encryptedRoom) {
			roomValue = legacySimpleDecrypt(decodeURIComponent(encryptedRoom));
			if (encryptedPwd) {
				pwdValue = legacySimpleDecrypt(decodeURIComponent(encryptedPwd));
			}
	...
	if (roomValue) {
		...
		// Clear URL parameters and fragments for security
