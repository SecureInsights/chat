...
// 分卷大小统一配置
const DEFAULT_VOLUME_SIZE = 256 * 1024;
const MAX_FILES_PER_BATCH = 20;
const MAX_SINGLE_FILE_SIZE = 50 * 1024 * 1024;
const MAX_TOTAL_FILE_SIZE = 100 * 1024 * 1024;
const MAX_TOTAL_VOLUMES = 512;
const MAX_VOLUME_DATA_LENGTH = Math.ceil(DEFAULT_VOLUME_SIZE * 4 / 3) + 128;
const FILE_ID_RE = /^file_[0-9]+_[A-Za-z0-9_-]{16,}$/;
...
function generateFileId() {
	...
	return 'file_' + Date.now() + '_' + btoa(binary).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/g, '');
}
		...
		reader.onload = async function(e) {
			...
			try {
				...
				deflate(arrayBuffer, { 
					...
					if (volumes.length > MAX_TOTAL_VOLUMES) {
						reject(new Error('File is too large after compression'));
						return;
					}
...
async function compressFilesToArchive(files, volumeSize = DEFAULT_VOLUME_SIZE) {	try {
		...
		return new Promise((resolve, reject) => {
			deflate(combinedData, { 
				...
				if (volumes.length > MAX_TOTAL_VOLUMES) {
					reject(new Error('Archive is too large after compression'));
					return;
				}
...
async function handleFilesUpload(files, onSend) {
	...
	if (!validateOutgoingFiles(files)) return;
...
function validateOutgoingFiles(files) {
	if (files.length > MAX_FILES_PER_BATCH) {
		window.addSystemMsg && window.addSystemMsg(`Too many files. Maximum ${MAX_FILES_PER_BATCH} files per batch.`);
		return false;
	}
	const totalSize = files.reduce((sum, file) => sum + file.size, 0);
	if (totalSize > MAX_TOTAL_FILE_SIZE) {
		window.addSystemMsg && window.addSystemMsg(`Files are too large. Maximum total size is ${formatFileSize(MAX_TOTAL_FILE_SIZE)}.`);
		return false;
	}
	for (const file of files) {
		if (file.size > MAX_SINGLE_FILE_SIZE) {
			window.addSystemMsg && window.addSystemMsg(`${file.name} is too large. Maximum file size is ${formatFileSize(MAX_SINGLE_FILE_SIZE)}.`);
			return false;
		}
	}
	return true;
...
function updateFileProgress(fileId) {
	...
	if (!transfer) return;
	const elements = Array.from(document.querySelectorAll('.file-message')).filter(element => element.dataset.fileId === fileId);
...
function handleFileVolume(message) {
	...
	if (!transfer) return;
	if (!Number.isInteger(volumeIndex) || volumeIndex < 0 || volumeIndex >= transfer.totalVolumes) return;
	if (typeof volumeData !== 'string' || volumeData.length > MAX_VOLUME_DATA_LENGTH) return;
...
function isValidIncomingStart(message) {
	if (!message || !FILE_ID_RE.test(String(message.fileId || ''))) return false;
	if (typeof message.fileName !== 'string' || message.fileName.length > 255) return false;
	if (!Number.isSafeInteger(message.originalSize) || message.originalSize < 0 || message.originalSize > MAX_TOTAL_FILE_SIZE) return false;
	if (!Number.isSafeInteger(message.compressedSize) || message.compressedSize < 0) return false;
	if (!Number.isInteger(message.totalVolumes) || message.totalVolumes < 1 || message.totalVolumes > MAX_TOTAL_VOLUMES) return false;
	if (message.fileCount && (!Number.isInteger(message.fileCount) || message.fileCount < 1 || message.fileCount > MAX_FILES_PER_BATCH)) return false;
	if (message.fileManifest && !Array.isArray(message.fileManifest)) return false;
	return true;
}
